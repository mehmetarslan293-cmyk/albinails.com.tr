...
const ADMIN_PASS = "albi2026";
const AUTH_KEY = "albiAdminAuth";
...
const tildaThumbnailSrc = (originalUrl, maxEdgePx = 96) => {
  ...
  try {
    ...
    const edge = Math.min(Math.max(Number(maxEdgePx) || 96, 48), 512);
...
/** Tilda albinails.com — HYPNOTIC COLLECTION (script.js ile aynı tohum). */
...
/** Tilda albinails.ru — Women koleksiyonu (script.js ile aynı tohum). */
...
/** Tilda albinails.ru — Classic koleksiyonu (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Gel polish ALBI Cat 5D (script.js ile aynı tohum). */
...
/** Tilda albinails.ru — Sonbahar koleksiyonu AUTUMN COLLECTION (5 ton; script.js ile aynı tohum). */
...
/** Tilda albinails.com — Star koleksiyonu STAR COLLECTION 1–19 (script.js ile aynı tohum). */
...
/** Tilda albinails.ru — Metal koleksiyonu METALLIC COLLECTION (8 ton; script.js ile aynı tohum). */
...
/** Tilda albinails.ru — Mozaic büyük sim / holo jel oje 405–412 (8 ton; script.js ile aynı tohum). */
...
/** Tilda albinails.ru — SILK CAT (Шёлковые кошки); script.js ile aynı tohum. */
...
/** Tilda albinails.com — Gel polish ALBI Shine Cat (script.js ile aynı tohum). */
...
/** Tilda albinails.com — French jel oje (01–09) + French manicure kit; script.js ile aynı tohum. */
...
/** Tilda albinails.ru — Men + Women sınırlı set (12 ton; script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Papaya (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Peach (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Banana (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Aqua (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Cloud (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Orchid (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Azure (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Innocent (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Champagne (script.js ile aynı tohum). */
...
/** Tilda — ALBI Sculptural gel Gentle (script.js ile aynı tohum). */
...
/** Tilda — ALBI Cream gel Beige (script.js ile aynı tohum). */
...
/** Tilda — ALBI Cream gel Rose (script.js ile aynı tohum). */
...
/** Tilda — 1+1 Sculptural gel NEW MILKY ROSE (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel 01–07 (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel RED series (script.js ile aynı tohum). */
...
/** Tilda — «Liquid Polygel» RED 01 kısa vitrin (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Inspire (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Almond Milk (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Blossom (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Sensitive (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Cashmere (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Toffee (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Candy Cotton (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Nude Inhale (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Tea Rose (script.js ile aynı tohum). */
...
/** Tilda — ALBI Liquid polygel Summer Collection 01–08 (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Starter kit (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Nail and Skin Care (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Nail Files (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Gel polish SUMMER 143–151 seti (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Gel polish SUMMER 159–169 seti (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Base Coat bölümü (script.js ile aynı tohum). */
...
/** Tilda albinails.com — Top Coat bölümü (script.js ile aynı tohum). */
...
const heroLeadInput = document.querySelector("#heroLeadInput");
const whyTitleInput = document.querySelector("#whyTitleInput");
const whyCardTitleInput = document.querySelector("#whyCardTitleInput");
const whyCardTextInput = document.querySelector("#whyCardTextInput");
...
const newBrandInput = document.querySelector("#newBrandInput");
...
const bannerPriceInput = document.querySelector("#bannerPrice");
const bannerTitleInput = document.querySelector("#bannerTitle");
const bannerDescriptionInput = document.querySelector("#bannerDescription");
const bannerButtonTextInput = document.querySelector("#bannerButtonText");
const bannerButtonUrlInput = document.querySelector("#bannerButtonUrl");
const bannerImageUrlInput = document.querySelector("#bannerImageUrl");
const bannerImageFileInput = document.querySelector("#bannerImageFile");
...
const metricCatalogViews = document.querySelector("#metricCatalogViews");
const metricAdminLogins = document.querySelector("#metricAdminLogins");

let pendingImageData = "";
let pendingBannerImageData = "";

const defaultBrands = ["Kalipso", "LOVELY", "MIO NAILS"];
...
const loadProducts = () => {
  ...
  try {
    ...
    const albiMig = migrateRemoveAlbiBrand(next);
    if (albiMig.changed) {
      changed = true;
      next = albiMig.next;
    }
...
const LASH_CATEGORY_ADMIN = "ipek-kirpik";
...
const normalizeLovelyBrandToken = (name) => {
  ...
  if (lower === "riley" || lower === "lashy" || lower.startsWith("lovely /")) return "LOVELY";
  return s;
};

const dedupeBrandList = (brands) => {
  const out = [];
  const seen = new Set();
  for (const raw of brands) {
    const n = normalizeLovelyBrandToken(raw);
    if (!n) continue;
    const lk = n.toLocaleLowerCase("tr");
...
const loadBrands = () => {
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length) {
      return dedupeBrandList([...new Set([...parsed, ...defaultBrands])]);
    }
...
const ratioText = (w, h) => {
  ...
  const ratio = w / h;
  const isValid = Math.abs(ratio - TARGET_BANNER_RATIO) <= RATIO_TOLERANCE;
  const state = isValid ? "Uygun" : "Uygun değil";
  return `Oran bilgisi: ${w}x${h} (${ratio.toFixed(2)}:1) - ${state} (Önerilen: 16:9)`;
};

const analyzeImageRatio = (src) =>
  new Promise((resolve) => {
    const img = new Image();
...
const renderBrandOptions = (selectedBrand = "Kalipso") => {
  if (!productBrandInput) return;
  const brands = loadBrands();
  productBrandInput.innerHTML = brands
    .map((brand) => `<option value="${brand}">${brand}</option>`)
    .join("");
  productBrandInput.value = brands.includes(selectedBrand) ? selectedBrand : brands[0];
};
  ...
  bannerList.innerHTML = banners
    .map(
      ...
      <article class="admin-banner-item">
        <img src="${b.image || "https://placehold.co/320x180/f1e5f8/6a4a74?text=Banner"}" alt="${b.title}" />
        <div>
          <strong>${b.title}</strong>
          <p>${b.subtitle}</p>
          <p>Buton: ${b.buttonText}</p>
        </div>
        <div class="admin-actions">
          <button type="button" data-banner-edit="${b.id}">Düzenle</button>
          <button type="button" class="danger" data-banner-delete="${b.id}">Sil</button>
        </div>
      </article>
    `
    )
...
const resetBannerForm = () => {
  ...
  bannerForm.reset();
  bannerIdInput.value = "";
  pendingBannerImageData = "";
  bannerButtonTextInput.value = "Detayları İncele";
  bannerButtonUrlInput.value = "./katalog.html?filter=all";
  if (bannerRatioInfo) bannerRatioInfo.textContent = "Oran bilgisi: Henüz seçilmedi.";
  if (saveBannerBtn) saveBannerBtn.textContent = "Banner Kaydet";
};
...
const loadMetrics = () => {
  ...
  try {
    const parsed = JSON.parse(stored);
    return { ...defaultMetrics, ...parsed, dailyVisits: { ...defaultMetrics.dailyVisits, ...(parsed.dailyVisits || {}) } };
  } catch {
...
const renderMetrics = () => {
  ...
  if (metricTotalVisits) metricTotalVisits.textContent = String(metrics.totalVisits || 0);
  if (metricTodayVisits) metricTodayVisits.textContent = String((metrics.dailyVisits || {})[today] || 0);
  if (metricHomeViews) metricHomeViews.textContent = String(metrics.homeViews || 0);
  if (metricCatalogViews) metricCatalogViews.textContent = String(metrics.catalogViews || 0);
  if (metricAdminLogins) metricAdminLogins.textContent = String(metrics.adminLogins || 0);
};
  ...
  productList.innerHTML = products
    .map(
      ...
      <article class="admin-product-item">
        <img src="${p.image ? tildaThumbnailSrc(p.image, 112) : "https://placehold.co/72x72/f4eef6/6d5a76?text=IMG"}" alt="${p.name}" width="72" height="72" loading="lazy" decoding="async" />
        ...
        <div class="admin-actions">
          ...
          <button type="button" class="danger" data-delete="${p.id}">Sil</button>
        </div>
      </article>
    `;
      }
...
const fillContentForm = () => {
  if (!heroTitleInput || !heroLeadInput || !whyTitleInput || !whyCardTitleInput || !whyCardTextInput) {
    return;
  }
  const content = loadContent();
  heroTitleInput.value = content.heroTitle || "";
  heroLeadInput.value = content.heroLead || "";
  whyTitleInput.value = content.whyTitle || "";
  whyCardTitleInput.value = content.whyCardTitle || "";
  whyCardTextInput.value = content.whyCardText || "";
};
...
if (bannerImageFileInput) {
  bannerImageFileInput.addEventListener("change", async (event) => {
    ...
    const reader = new FileReader();
    reader.onload = async () => {
      pendingBannerImageData = String(reader.result || "");
      const ratio = await analyzeImageRatio(pendingBannerImageData);
      if (bannerRatioInfo) bannerRatioInfo.textContent = ratioText(ratio.width, ratio.height);
    };
    reader.readAsDataURL(file);
  });
}

if (bannerImageUrlInput) {
  bannerImageUrlInput.addEventListener("blur", async () => {
    const url = bannerImageUrlInput.value.trim();
    if (!url) return;
    const ratio = await analyzeImageRatio(url);
    if (bannerRatioInfo) bannerRatioInfo.textContent = ratioText(ratio.width, ratio.height);
  });
}
...
if (productForm) {
  productForm.addEventListener("submit", (event) => {
    ...
    const payload = {
      ...
      brand: productBrandInput.value.trim() || "Kalipso",
      size,
...
if (productCategoryInput) {
  productCategoryInput.addEventListener("change", () => syncProductSizeHint());
}
...
if (bannerForm) {
  bannerForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const banners = loadBanners();
    const editId = bannerIdInput.value;
    const payload = {
      id: editId || `b_${Date.now()}`,
      subtitle: bannerSubtitleInput.value.trim(),
      price: bannerPriceInput.value.trim(),
      title: bannerTitleInput.value.trim(),
      description: bannerDescriptionInput.value.trim(),
      buttonText: bannerButtonTextInput.value.trim() || "Detayları İncele",
      buttonUrl: bannerButtonUrlInput.value.trim() || "./katalog.html?filter=all",
      image: pendingBannerImageData || bannerImageUrlInput.value.trim(),
    };

    if (editId) {
      const idx = banners.findIndex((b) => b.id === editId);
      if (idx >= 0) banners[idx] = { ...banners[idx], ...payload };
    } else {
      banners.unshift(payload);
    }
...
if (bannerList) {
  bannerList.addEventListener("click", (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    const editId = target.dataset.bannerEdit;
    const deleteId = target.dataset.bannerDelete;
    const banners = loadBanners();

    if (deleteId) {
      const next = banners.filter((b) => b.id !== deleteId);
      saveBanners(next.length ? next : [...defaultBanners]);
      renderBanners();
      return;
    }
    ...
    if (editId) {
      ...
      if (!item) return;
      bannerIdInput.value = item.id;
      bannerSubtitleInput.value = item.subtitle;
      bannerPriceInput.value = item.price;
      bannerTitleInput.value = item.title;
      bannerDescriptionInput.value = item.description;
      bannerButtonTextInput.value = item.buttonText;
      bannerButtonUrlInput.value = item.buttonUrl;
      bannerImageUrlInput.value = item.image || "";
      pendingBannerImageData = item.image || "";
      if (bannerRatioInfo) bannerRatioInfo.textContent = "Oran bilgisi: Düzenleme modunda.";
      if (saveBannerBtn) saveBannerBtn.textContent = "Banner Güncelle";
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  });
}
...
if (addBrandBtn) {
  addBrandBtn.addEventListener("click", () => {
    ...
    renderBrandOptions(name);
    if (newBrandInput) newBrandInput.value = "";
  });
}

[filterProductNameInput, filterBrandInput].forEach((input) => {
  if (!input) return;
...
if (contentForm) {
  contentForm.addEventListener("submit", (event) => {
    ...
    saveContent({
      ...
      heroLead: heroLeadInput.value.trim(),
      whyTitle: whyTitleInput.value.trim(),
      whyCardTitle: whyCardTitleInput.value.trim(),
      whyCardText: whyCardTextInput.value.trim(),
    });
    alert("Metinler kaydedildi. Siteyi yenileyerek görebilirsiniz.");
  });
}
